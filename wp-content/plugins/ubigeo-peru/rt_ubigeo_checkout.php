    ...
    <script>
        jQuery(document).ready(function () {
            ...
            function rt_ubigeo_event_departamento(select, selectType) {
                ...
                const id = jQuery(select)[0].selectedIndex;
                const value = jQuery(select).find('option').eq(id).text().toUpperCase();
                ...
                jQuery.ajax({
                    ...
                    success: function (response) {
                        jQuery('#' + selectType + '_provincia').html('');
                        if (response) {
                            jQuery('#' + selectType + '_provincia').append('<option value="">Seleccionar Provincia</option>');
                            ...
                            for (let i=0;i<who.length;i++) {
                                if (value == who.eq(i).text().toUpperCase()) {
                                    const valor = who.eq(i).attr('value');
                                    jQuery('#billing_who_dep').val(valor);
                                    jQuery('#billing_who_dep').trigger('change');
                                }
                            }
                        }
                    },
            ...
            function rt_ubigeo_event_provincia(select, selectType) {
                const id = jQuery(select)[0].selectedIndex;
                const value = jQuery(select).find('option').eq(id).text().toUpperCase().replace(/ /g, "");
                var data = {
                ...
                jQuery.ajax({
                    ...
                    success: function (response) {
                        jQuery('#' + selectType + '_distrito').html('');
                        if (response) {
                            jQuery('#' + selectType + '_distrito').append('<option value="">Seleccionar Distrito</option>');
                            for (var r in response) {
                                jQuery('#' + selectType + '_distrito').append('<option value=' + r + '>' + response[r] + '</option>')
                            }
                            //add to woof                            
                            const who = jQuery('#billing_city option');
                            for (let i=0;i<who.length;i++) {
                                if (value == who.eq(i).text().toUpperCase().replace(/ /g, "")) {
                                    const valor = who.eq(i).attr('value');
                                    jQuery('#billing_city').val(valor);
            ...
            jQuery('#billing_distrito, #shipping_distrito').on('change', function () {
                ...
                const id = jQuery('#billing_distrito')[0].selectedIndex;
                const value = jQuery('#billing_distrito').find('option').eq(id).text().toUpperCase().replace(/ /g, "");                
                //add to woof                            
                const who = jQuery('#billing_postcode option');
                for (let i=0;i<who.length;i++) {
                    if (value == who.eq(i).text().toUpperCase().replace(/ /g, "")) {
                        const valor = who.eq(i).attr('value');
                        jQuery('#billing_postcode').val(valor);
